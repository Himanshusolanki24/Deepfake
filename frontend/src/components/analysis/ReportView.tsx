    ...
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => { toast.success("PDF report queued for download"); }}>
      ...
      <article className="space-y-8 rounded-lg border border-border bg-card p-6 shadow-sm sm:p-10">
        <header className="border-b border-border pb-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-xl font-semibold text-foreground">Forensic Media Assessment Report</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                Case <span className="hex-mono font-semibold text-foreground">{result.id}</span> · {result.filename}
              </p>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Generated</p>
              <p className="hex-mono text-xs text-foreground tabular">
                {new Date(result.createdAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}
              </p>
              <p className="mt-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Report ID</p>
        ...
        <section aria-label="Media information">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg border border-border bg-muted/30 p-5 sm:grid-cols-4">
            ...
            {result.confidenceInterval && (
              <ReportField label="Interval" value={`${result.confidenceInterval.lower}–${result.confidenceInterval.upper}%`} mono />
            )}
                  ...
                  {result.suspiciousFrames
                    ...
                    .map((f) => (
                      <tr key={f.frame} className="border-t border-border">
                        <td className="hex-mono px-4 py-2.5 font-semibold">{String(f.frame).padStart(4, "0")}</td>
                        <td className="hex-mono px-4 py-2.5 text-muted-foreground tabular">{formatSeconds(f.timestamp)}</td>
                        <td className="hex-mono px-4 py-2.5 font-semibold text-manipulated">{f.score.toFixed(2)}</td>
        ...
        <section aria-label="Methodology">
          ...
          <ol className="list-decimal space-y-1.5 pl-5 text-sm leading-6 text-foreground/80">
            ...
            <li>Independent signal detectors run over the media: spatial artifacts, frequency-domain analysis, temporal consistency, physiological proxies, audio-visual synchronization and metadata forensics.</li>
        ...
        <footer className="border-t border-border pt-4 text-center">
          <p className="text-[11px] text-muted-foreground">
            Generated by AUTHENTIQ Digital Forensics · Engine v2.4.1 · Verdict: {VERDICT_HEADLINES[result.verdict]}
          </p>
        </footer>
      </article>
    </div>
...
function ReportField({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="min-w-0">
      ...
      <dd className={mono ? "hex-mono truncate text-[13px] font-medium text-foreground" : "truncate text-[13px] font-medium text-foreground"}>
        {value}
      </dd>
    </div>
  );
