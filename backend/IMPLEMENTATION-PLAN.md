...
7. **Registry/adapters** (spec §13): ~~expand `ModelSpec`; `ModelAdapter` Protocol + `predict()`; compose per-family adapters~~ DONE (`ml/model_registry.py` 12 models + `ENGINE_VERSION`; `ml/adapters.py` TFKeras/Onnx/Mock; `ml/inference.py` `model_version` on Protocols). Migrated from PyTorch to TensorFlow 2.x CPU.
